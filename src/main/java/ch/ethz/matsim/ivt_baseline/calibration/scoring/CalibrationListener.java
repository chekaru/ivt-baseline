package ch.ethz.matsim.ivt_baseline.calibration.scoring;
...
public class CalibrationListener implements IterationEndsListener, ShutdownListener {
    ...
    private void applyBinDistances(List<String> row) {
        for (int i = 0; i < 3 * modes.size() + 1; i++) {
            row.add("");
        }

        for (String mode : modes) {
            for (int i = 0; i < numberOfDistanceBins; i++) {
                row.add(String.valueOf((double) (i+1) * (maximumDistance / numberOfDistanceBins)));
            }
        }
    }
